...
def user_input(user_question):
    ...
    if os.path.exists("faiss_index"):
        try:
            # Allow dangerous deserialization while loading the FAISS index
...
def main():
    st.set_page_config("PDF Insight")
    ...
    with st.sidebar:
        ...
        if st.button("Get Key Points"):
            input_prompt = """Fetch all key points in detail from given text information. Do not give extra other information; only give information relevant to the text information."""
            raw_text = get_pdf_text(pdf_docs)
            if raw_text:
                response = get_gemini_repsonse(raw_text + " as text information " + input_prompt)
                st.subheader("Key Points:")
                st.write(response)
            else:
                st.error("No text information available for key points.")
